...
def tExport(processedTrace, ratName, experimentDate):
    abfWriter.writeABF1(sweepData= processedTrace, filename= os.path.join(os.getcwd(), "Processed Data", "%s Rat %i Pre-Injection Peaks.xlsx"%(experimentDate.strftime("%Y-%m-%d"), int(ratName))), units="V", sampleRateHz= 3333.33)

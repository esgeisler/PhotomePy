...
#     def __init__(self, filename, processedSignalArray):
...
# class Peaks(ProcessedTotalSignal):
#     def __init__(self, filename, processedSignalArray):
#         super().__init__(filename, processedSignalArray)

#         self.processedSignalArray = processedSignalArray
#         self.fileName = filename
#         self.peaksDict = {}
#         self.mostPeaksInTrace = self.peakMax()
#         self.peaksArray = np.zeros((self.numTraces, self.mostPeaksInTrace))

#         self.widthBottomArray = np.zeros((self.numTraces, 4, self.mostPeaksInTrace))
#         self.width10Array = np.zeros((self.numTraces, 4, self.mostPeaksInTrace))
#         self.widthHalfArray = np.zeros((self.numTraces, 4, self.mostPeaksInTrace))
#         self.width90Array = np.zeros((self.numTraces, 4, self.mostPeaksInTrace))

#     def peakLocator(self):
#         for index, traces in enumerate(self.processedSignalArray): # Finds peaks in a signal
#             peaks, self.peaksDict[index] = sci.find_peaks(traces, prominence= 0.05, width=0, wlen=20000, rel_height= 0.5)
#             bottomWidth = sci.peak_widths(traces, peaks, rel_height=1, prominence_data=(self.peaksDict[index]['prominences'], self.peaksDict[index]["left_bases"], 
#                                                                                         self.peaksDict[index]["right_bases"]), wlen=20000)
#             width10 = sci.peak_widths(traces, peaks, rel_height=0.1, prominence_data=(self.peaksDict[index]['prominences'], 
#                                                                                     self.peaksDict[index]["left_bases"], self.peaksDict[index]["right_bases"]), wlen=20000)
#             widthHalf = sci.peak_widths(traces, peaks, rel_height=0.5, prominence_data=(self.peaksDict[index]['prominences'], 
#                                                                                         self.peaksDict[index]["left_bases"], self.peaksDict[index]["right_bases"]), wlen=20000)
#             width90 = sci.peak_widths(traces, peaks, rel_height=0.9, prominence_data=(self.peaksDict[index]['prominences'], 
#                                                                                     self.peaksDict[index]["left_bases"], self.peaksDict[index]["right_bases"]), wlen=20000)
...
#     def peakOverlapAndDegree(self):
#         overlapPeaks = [[0]*self.mostPeaksInTrace for _ in self.abf.sweepList]
#         overlapRise =[[0]*self.mostPeaksInTrace for _ in self.abf.sweepList]
#         overlapDecay = [[0]*self.mostPeaksInTrace for _ in self.abf.sweepList]
...
#         for k, checkPeak in np.ndenumerate(self.peaksArray):
#             overlapPeaks[k[0]][k[1]] = np.array([y for y in self.peaksArray[k[0]] if ((self.widthBottomArray[k[0]][2][k[1]] < y < self.widthBottomArray[k[0]][3][k[1]]) and y != checkPeak)])
#             overlapRise[k[0]][k[1]] = np.array([y for y in self.peaksArray[k[0]] if ((self.widthBottomArray[k[0]][2][k[1]] < y < checkPeak) and y != checkPeak)])
#             overlapDecay[k[0]][k[1]] = np.array([y for y in self.peaksArray[k[0]] if ((checkPeak < y < self.widthBottomArray[k[0]][3][k[1]]) and y != checkPeak)])
            
#             if len(self.processedSignalArray[k[0]][int(self.widthBottomArray[k[0]][2][k[1]]):int(self.widthBottomArray[k[0]][3][k[1]])]) == 0:
#                 continue
#             # Put overlapPeaks into a list first, then put that into degreeNPeaks? Check later
#             self.degreeNPeaks[peakOfDegree] = len(overlapPeaks[k[0]][k[1]])
#             self.riseNPeaks[peakOfDegree] = len(overlapRise[k[0]][k[1]])
#             self.decayNPeaks[peakOfDegree] = len(overlapDecay[k[0]][k[1]])
#         self.sortedDegreeNPeaks = dict(sorted(self.degreeNPeaks.items(), key=lambda item: item[1]))
#         self.sortedDegreeRise = dict(sorted(self.riseNPeaks.items(), key=lambda item: item[1]))
#         self.sortedDegreeDecay = dict(sorted(self.decayNPeaks.items(), key=lambda item: item[1]))
#         return self.sortedDegreeNPeaks, self.sortedDegreeRise, self.sortedDegreeDecay

# class PeaksInTrace(Peaks):
#     def __init__(self, filename, processedSignalArray, fileIndex):
#         super().__init__(filename, processedSignalArray)

#         self.fileName = filename
#         self.processedSignalArray = processedSignalArray
#         self.fileIndex = fileIndex
#         self.peaksIn = self.peaksArray[fileIndex]

#         self.degreeNPeaks, self.riseNPeaks, self.decayNPeaks = {}, {}, {}
#         self.sortedDegreeNPeaks, self.sortedDegreeRise, self.sortedDegreeDecay = {}, {}, {}

#         self.overlapInfo = self.peakOverlapCheck()
#         self.adjustedArea = np.zeros((self.numTraces, self.mostPeaksInTrace))

#         self.Event_Num = [x + 1 for x, _ in enumerate(self.peaksIn)]
#         self.Peak_Index = self.peaksIn
#         self.Peak_Time_Sec = ((self.peaksIn/self.samplingFreqSec) + (self.fileIndex * 30)).round(2)
#         self.Event_Window_Start = self.peaksDict[self.fileIndex]['left_ips'].round(2)
#         self.Event_Window_End = self.peaksDict[self.fileIndex]['right_ips'].round(2)
#         self.Amplitude = self.peaksDict[self.fileIndex]["prominences"].round(3)
#         self.Off_Time_ms = ((self.peaksDict[self.fileIndex]['right_bases'] - self.peaksIn)/(self.samplingFreqMSec)).round(2)
#         self.Width_at50_ms = (self.peaksDict[self.fileIndex]['widths']/(self.samplingFreqMSec)).round(2)
#         self.Avg_Area = self.areaAdjuster()

#     def peakDegreeFinder(self):
#         for i, peakOfDegree in enumerate(self.peaksIn): 
#             if len(self.processedSignalArray[self.fileIndex][int(self.widthBottomArray[self.fileIndex][2][i]):int(self.widthBottomArray[self.fileIndex][3][i])]) == 0:
#                 continue
#             self.degreeNPeaks[peakOfDegree] = len(self.overlapInfo[0][self.fileIndex][i])
#             self.riseNPeaks[peakOfDegree] = len(self.overlapInfo[1][self.fileIndex][i])
#             self.decayNPeaks[peakOfDegree] = len(self.overlapInfo[2][self.fileIndex][i])
#         self.sortedDegreeNPeaks = dict(sorted(self.degreeNPeaks.items(), key=lambda item: item[1]))
#         self.sortedDegreeRise = dict(sorted(self.riseNPeaks.items(), key=lambda item: item[1]))
#         self.sortedDegreeDecay = dict(sorted(self.decayNPeaks.items(), key=lambda item: item[1]))
#         return self.sortedDegreeNPeaks, self.sortedDegreeRise, self.sortedDegreeDecay
    
#     def areaAdjuster(self):
#         for _, u in enumerate(self.sortedDegreeNPeaks):
#             i = np.where(self.peaksIn == u)
#             if len(self.processedSignalArray[self.fileIndex][int(self.widthBottomArray[self.fileIndex][2][i[0][0]]):int(self.widthBottomArray[self.fileIndex][3][i[0][0]])]) == 0:
#                 continue
#             peaksInArea = self.overlapInfo[0][self.fileIndex][i[0][0]]
#             peakArea = inte.simpson(y=self.processedSignalArray[self.fileIndex][int(self.widthBottomArray[self.fileIndex][2][i[0][0]]):int(self.widthBottomArray[self.fileIndex][3][i[0][0]])], 
#                                     x=np.arange(int(self.widthBottomArray[self.fileIndex][2][i[0][0]]), int(self.widthBottomArray[self.fileIndex][3][i[0][0]]))/self.samplingFreqMSec)
#             match self.degreeNPeaks[u]:
#                 case 0:
#                     self.adjustedArea[self.fileIndex][i[0][0]] = peakArea.round(2)
#                 case _:
#                     for l in peaksInArea:
#                         j = np.where(self.peaksIn == l)
#                         peakArea -= self.adjustedArea[self.fileIndex][j[0][0]]
#                     self.adjustedArea[self.fileIndex][i[0][0]] = peakArea.round(2)
#         return pd.Series(self.adjustedArea[self.fileIndex])
